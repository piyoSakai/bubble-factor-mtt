...
const createDrillRound = (presetId: string): DrillRound | null => {
  const preset = PRESET_DEFS.find((item) => item.id === presetId);
  if (!preset) {
    return null;
  }

  const input = buildInputFromPreset(preset, 'bb');
  const result = calculateAll(input);
  const callerIndex = getRandomCallerIndex(input.players.length);
...
function DrillPage() {
  const initialRound = useMemo(() => createDrillRound(DEFAULT_PRESET_ID), []);
  ...
  const startRound = useCallback((presetId: string) => {
    const nextRound = createDrillRound(presetId);
    if (!nextRound) {
      return;
    }
  ...
  const handleSubmit = () => {
    ...
    setBestScore((current) => (current === null ? summary.roundScore : Math.max(current, summary.roundScore)));
      ...
      <section className="hero">
        <div>
          ...
          <p className="hero-copy">
            Caller 1人の行を隠して出題。推測値を入力して Submit で採点します。
          </p>
        </div>
      ...
      <section className="panel">
        <div className="panel-head">
          ...
          <div className="action-row">
            <button
              ...
              onClick={() => startRound(selectedPresetId)}
            >
              New round
            </button>
            <button
        ...
        {caller ? (
          <p className="helper-copy">
            Caller: <strong>{caller.name}</strong> ({stackFormatter.format(caller.stack)} BB)
          </p>
        ) : null}
      </section>
        ...
        <section className="panel">
          <div className="panel-head">
            <div>
              ...
              <p className="helper-copy">対戦相手ごとに BF を入力してください（caller 自身は対象外）。</p>
            </div>
          </div>
              ...
              return (
                <div className="input-row drill-row" key={`${player.id}-drill`}>
                  ...
                  <div className="drill-result">
                    ...
                    <strong>{submitted && scored ? scoreFormatter.format(scored.cellScore) : '-'}</strong>
                  </div>
                </div>
              );
            })}
          ...
          {submitted && scoreSummary ? (
            <div className="warning-box drill-score-box" role="status">
              ...
              <p>
                Streak rule: all cells within {scoreFormatter.format(STREAK_SUCCESS_THRESHOLD * 100)}%
                relative error.
              </p>
            </div>
          ) : null}
